...
lib = ct.cdll.LoadLibrary(os.path.dirname(__file__)+'/lib/_plant_dyn_syn.so')
PI2 = tl.PI2
...
def integrate_n_rk4_phasedifferences_electricExcitatoryAndInhibitoryConnections(networkSize, initial_states,
                                     ...
                                     coupling_alpha_syn, coupling_dyn_Inh, coupling_dyn_Exc, coupling_ftm,
                                     coupling_alpha_syn_si3,coupling_alpha_syn_cross,coupling_alpha_syn_ipsi,
    ...
    coupling_alpha_syn = np.asarray(coupling_alpha_syn)
    assert coupling_alpha_syn.size == networkSize * networkSize
    coupling_dyn_Inh = np.asarray(coupling_dyn_Inh)
    assert coupling_dyn_Inh.size == networkSize * networkSize
    coupling_dyn_Exc = np.asarray(coupling_dyn_Exc)
    assert coupling_dyn_Exc.size == networkSize * networkSize
    coupling_ftm = np.asarray(coupling_ftm)
    assert coupling_ftm.size == networkSize * networkSize
    coupling_alpha_syn_si3 = np.asarray(coupling_alpha_syn_si3)
    assert coupling_alpha_syn_si3.size == networkSize * networkSize
    coupling_alpha_syn_cross = np.asarray(coupling_alpha_syn_cross)
    assert coupling_alpha_syn_cross.size == networkSize * networkSize
    coupling_alpha_syn_ipsi = np.asarray(coupling_alpha_syn_ipsi)
    assert coupling_alpha_syn_ipsi.size == networkSize * networkSize
    ...
    lastSpikes = np.zeros(point_dimension/2, int)
    burstLength = int(4000/dt)
    ...
    lib.integrate_n_rk4_phasedifferences_electricExcitatoryAndInhibitoryConnections(ct.c_uint(networkSize), initial_states.ctypes.data_as(ct.POINTER(ct.c_double)),
                                                                     ...
                                                                     coupling_alpha_syn.ctypes.data_as(ct.POINTER(ct.c_double)),
                                                                     coupling_dyn_Inh.ctypes.data_as(ct.POINTER(ct.c_double)),
                                                                     coupling_dyn_Exc.ctypes.data_as(ct.POINTER(ct.c_double)),
                                                                     coupling_ftm.ctypes.data_as(ct.POINTER(ct.c_double)),
                                                                     coupling_alpha_syn_si3.ctypes.data_as(ct.POINTER(ct.c_double)),
                                                                     coupling_alpha_syn_cross.ctypes.data_as(ct.POINTER(ct.c_double)),
                                                                     coupling_alpha_syn_ipsi.ctypes.data_as(ct.POINTER(ct.c_double)),
                                                                     X_out.ctypes.data_as(ct.POINTER(ct.c_double)),
                                                                     ...
                                                                     lastSpikes.ctypes.data_as(ct.POINTER(ct.c_int)), ct.c_uint(burstLength),
    ...
    phaseDiffs = np.reshape(P_out, (numberOfPhaseDifferences[0], networkSize-1), 'C'), ', size : ', numberOfPhaseDifferences[0]
    #np.asarray(phaseDiffs[0]).tofile('yourfile.txt', sep=" ", format="%s")
    np.savetxt('pd.csv', np.asarray(phaseDiffs[0]), newline="\n", delimiter=',')
    print phaseDiffs
...
def parameters_generic(networkSize, parameter_input_array=None):
    # E_syn_alpha	-80, shift	-57, alpha	.01, beta	0.001

    if parameter_input_array == None :
        parameter_array_onecell = np.array([-80, -57, 0.01, 0.001])
    else :
