...
#define THRESHOLD_SLOPE	10.	// mV^-1
...
#pragma acc routine seq
double x_inf(const double V)		{ return ((1/(exp(-0.3*(V+40))+1))); }     /*  { return (1/(exp(0.15*(-V-50))+1)); }*/
...
void derivs_one_electricExcitatoryAndInhibitoryConnections(double* y, double* dydt, const double* p)
{
	...
	dydt[0] = -g_Na*pow(m_inf(V_tilde), 3)*h*(V-V_Na) - g_Ca*x*(V-V_Na) - (g_K*pow(n, 4)+g_K_Ca*Ca/(0.5+Ca))*(V-V_K) - g_L*(V-V_L);	// dV/dt
                                ...
                                double* bm_factor)
{
	...
	for(i=0; i<n; i++)
	{
		...
		//TODO : p[4] is E_Cl; p[5] is E_K; p[6] is E_Na;
		   dydt[i*N_EQ1] += ((E_syn-y[i*N_EQ1])*coupling_strengths_chlorine[j*n+i] +
		                        (V_K-y[i*N_EQ1])*coupling_strengths_potassium[j*n+i] +
		                            (V_Na-y[i*N_EQ1])*coupling_strengths_sodium[j*n+i] )
		                             * bm_factor[j]
