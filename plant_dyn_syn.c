...
{
	double V=y[0], h=y[1], n=y[2], x=y[3], Ca=y[4], S=y[5], z=y[6], X=y[7], Y=y[8], M=y[9], S_e=y[10], S_3=y[11],S_cross=y[12], S_ipsi=y[13] ;
	double V_tilde = C_1*V+C_2;// V_Ca=p[0];

	    dydt[0] = -g_Na*pow(m_inf(V_tilde), 3)*h*(V-V_Na) - g_Ca*x*(V-V_Na) - (g_K*pow(n, 4)+g_K_Ca*Ca/(0.5+Ca))*(V-V_K) - p[11]*(V-p[12]) - p[8]*(pow((1./(1.+exp(-(V+63.)/7.8))),3)*z*(V - 120.)) + p[5]*((currentTime>p[6]&&currentTime<p[7])?1:0);	// dV/dt
        ...
        dydt[6] = 0.5*((1./(1+exp(10*(V+55.))))-z)/(7.1+10.4/(1+exp((V+68.)/2.2)));
        dydt[7] = 1; //X
        dydt[8] = (X-Y)/tauY; //Y
        dydt[9] = 0.; //M
        dydt[10] = 0.05 * (1. - S_e)/(1.+exp(-10.*(V-th))) - 0.0003*S_e;  // exc dyn alpha syn
        dydt[11] = 0.02 * (1. - S_3)/(1.+exp(-10.*(V-th))) - 0.002*S_3;  // 33
        dydt[12] = 0.02 * (1. - S_cross)/(1.+exp(-10.*(V-th))) - 0.003*S_cross;  //cross
        dydt[13] = 0.05 * (1. - S_ipsi)/(1.+exp(-10.*(V-th))) - 0.001*S_ipsi;  //ipsi
...
void derivs_n_variablecoupling_electricExcitatoryAndInhibitoryConnections(const unsigned n, double* y, double* dydt,
                                ...
                                const double* coupling_ftm, const double* coupling_alpha_syn_si3, const double* coupling_alpha_syn_cross,
                                const double* coupling_alpha_syn_ipsi, double* bm_factor, double currentTime)
{
	unsigned i, j;
	for(i=0;i <n; i++){
	...
	{
		for(j=0;j<n;j++){
		    dydt[i*N_EQ1 + 9] += (((1./(1+exp(-(y[j*N_EQ1]+40))))-y[i*N_EQ1+9])/tauM)*coupling_dyn_Exc[i*n+j]*40;
		}
	}

	for(i=0;i<n;i++)
			for(j=0;j<n;j++){
        		   //dydt[j*N_EQ1 + 9] *= (((1./(1+exp(-(y[i*N_EQ1]+40))))-y[j*N_EQ1+9])/tauM)*coupling_dyn_Exc[j*n+i]*40;

		           dydt[i*N_EQ1] += (p[i*NP1+0] - y[i*N_EQ1]) * coupling_alpha_syn[j*n+i] * y[j*N_EQ1+5]+
		                    (E_Na_Exc  - y[i*N_EQ1]) * coupling_strengths_sodium[j*n+i] * y[j*N_EQ1+10]+
		                    (E_dyn_Inh - y[i*N_EQ1]) * coupling_dyn_Inh[j*n+i] *y[j*N_EQ1+5] *y[i*N_EQ1+9]+
		                    (-10 - y[i*N_EQ1]) * coupling_dyn_Exc[j*n+i] *y[j*N_EQ1+10] *y[j*N_EQ1+9]+
					        (y[j*N_EQ1] - y[i*N_EQ1]) * coupling_strengths_electric[j*n+i]+
					        (p[i*NP1+0] - y[i*N_EQ1]) * coupling_ftm[j*n+i] / (1.+ exp(-1.*(y[j*N_EQ1]+50)))+
					        (-55 - y[i*N_EQ1]) * coupling_alpha_syn_si3[j*n+i] * y[j*N_EQ1+11]+
					        (-80 - y[i*N_EQ1]) * coupling_alpha_syn_cross[j*n+i] * y[j*N_EQ1+12]+
					        (-55 - y[i*N_EQ1]) * coupling_alpha_syn_ipsi[j*n+i] * y[j*N_EQ1+13];
		}
}
...
void integrate_n_rk4_phasedifferences_electricExcitatoryAndInhibitoryConnections(const unsigned networkSize,
                    ...
                    const double* coupling_alpha_syn, const double* coupling_dyn_Inh, const double* coupling_dyn_Exc,
                    const double* coupling_ftm, const double* coupling_alpha_syn_si3, const double* coupling_alpha_syn_cross,
                    const double* coupling_alpha_syn_ipsi,
    ...
    int burstInitiationTimes[networkSize][100];
    int burstInitiationTimesCounters[networkSize];
    int burstEndTimes[networkSize][100];
    int burstEndTimesCounters[networkSize];
    int burstDurations[networkSize];
	...
	for(m=0; m<point_dimension; m++){
		...
		//printf("%lf\n",current_state[m]);
            ...
            burstInitiationTimesCounters[m/N_EQ1]=0;
            burstEndTimesCounters[m/N_EQ1]=0;
	    }
	}
	...
	for(i=1; i<N; i++)
	{
		for(j=0; j<stride; j++)
		{
			derivs_n_variablecoupling_electricExcitatoryAndInhibitoryConnections(networkSize, current_state, k1, p, coupling_strengths_chlorine, coupling_strengths_potassium, coupling_strengths_sodium, coupling_strengths_electric, coupling_alpha_syn, coupling_dyn_Inh, coupling_dyn_Exc, coupling_ftm, coupling_alpha_syn_si3, coupling_alpha_syn_cross, coupling_alpha_syn_ipsi, bm_factor, currentTime);

			for(m=0; m<point_dimension; m++)
				temp_state[m] = current_state[m]+k1[m]*dt/2;
			derivs_n_variablecoupling_electricExcitatoryAndInhibitoryConnections(networkSize, temp_state, k2, p, coupling_strengths_chlorine, coupling_strengths_potassium, coupling_strengths_sodium, coupling_strengths_electric, coupling_alpha_syn, coupling_dyn_Inh, coupling_dyn_Exc, coupling_ftm,coupling_alpha_syn_si3, coupling_alpha_syn_cross, coupling_alpha_syn_ipsi, bm_factor, currentTime);

			for(m=0; m<point_dimension; m++)
				temp_state[m] = current_state[m]+k2[m]*dt/2;
			derivs_n_variablecoupling_electricExcitatoryAndInhibitoryConnections(networkSize, temp_state, k3, p, coupling_strengths_chlorine, coupling_strengths_potassium, coupling_strengths_sodium, coupling_strengths_electric, coupling_alpha_syn, coupling_dyn_Inh, coupling_dyn_Exc, coupling_ftm, coupling_alpha_syn_si3,coupling_alpha_syn_cross, coupling_alpha_syn_ipsi, bm_factor, currentTime);

			for(m=0; m<point_dimension; m++)
				temp_state[m] = current_state[m]+k3[m]*dt;
			derivs_n_variablecoupling_electricExcitatoryAndInhibitoryConnections(networkSize, temp_state, k4, p, coupling_strengths_chlorine, coupling_strengths_potassium, coupling_strengths_sodium, coupling_strengths_electric, coupling_alpha_syn, coupling_dyn_Inh, coupling_dyn_Exc, coupling_ftm, coupling_alpha_syn_si3,coupling_alpha_syn_cross, coupling_alpha_syn_ipsi, bm_factor, currentTime);
                ...
                if(m==0){ // first cell
                    if(current_state[m]>=threshhold && output[(i-1)*point_dimension/N_EQ1+m/N_EQ1] < threshhold){
                        if(i-lastSpikes[m/N_EQ1] > burstLength){
                            ...
                            if(allCellsHadZeroPhaseBefore){
                                if(burstInitiationTimesCounters[0]!=0)
                                    burstEndTimes[0][burstEndTimesCounters[0]++]=lastSpikes[m/N_EQ1];
                                burstInitiationTimes[0][burstInitiationTimesCounters[0]++]=i;
                                //printf("%d\t",i);
                                for(j=1;j<point_dimension/N_EQ1;j++){
                    ...
                    if(current_state[m]>=threshhold && output[(i-1)*point_dimension/N_EQ1+m/N_EQ1] < threshhold){
                        if(i-lastSpikes[m/N_EQ1] > burstLength){
                            if(!allCellsHadZeroPhaseBefore){
                                ...
                                if(burstInitiationTimesCounters[m/N_EQ1]!=0)
                                    burstEndTimes[m/N_EQ1][burstEndTimesCounters[m/N_EQ1]++]=lastSpikes[m/N_EQ1];
                                burstInitiationTimes[m/N_EQ1][burstInitiationTimesCounters[m/N_EQ1]++]=i;
                                //printf("%d\t",i);
                                //if(m/N_EQ1==networkSize-1)printf("\n");
    ...
    *numberOfPhaseDifferences = phase_differences_output_index - numberOfCrossingsUnedited;
    ...
    //printBurstInitiationTimes
    printf("Burst initiation times : \n");
    for(i=0;i<networkSize;i++){
        for(j=0;j<burstInitiationTimesCounters[i];j++){
            printf("%d\t",burstInitiationTimes[i][j]);
        }
        printf("\n");
    }
    printf("\n");
    ...
    //printBurstEndTimes
    printf("Burst End times : \n");
    for(i=0;i<networkSize;i++){
        for(j=0;j<burstEndTimesCounters[i];j++){
            printf("%d\t",burstEndTimes[i][j]);
        }
        printf("\n");
    }
        printf("\n");
    ...
    //printBurstDurations
    printf("Burst Durations : \n");
    for(i=0;i<networkSize;i++){
        for(j=0;j<burstEndTimesCounters[i];j++){
            printf("%d\t",burstEndTimes[i][j]-burstInitiationTimes[i][j]);
        }
        printf("\n");
    }
        printf("\n");
